...
import { weatherIcons, WeatherIconsKeysProps } from '@/utils/weather-icons'
...
export async function getWeatherByCity({
  ...
  const { data } = await api.get<GetWeatherByCityResponse>(
  ...
  data.list.forEach((item) => {
    ...
    if (days.includes(day) && !daysAdded.includes(day)) {
      ...
      nextDays.push({
        day: dayjs(new Date(item.dt_txt)).locale('pt-br').format('ddd'),
